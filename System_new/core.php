...
defined('APP_PATH') or define('APP_PATH',dirname($_SCRIPT['SCRIPT_FILENAME']).'/');
...
defined('LIB_PAHT') or define('LIB_PATH',realpath(THINK_PATH.'Library').'/');
defined('MODE_PAHT') or define('MODE_PATH',THINK_PATH.'Mode/');
...
defined('CONF_RARSE') or define('CONF_PRASE','');
...
if(!IS_CLI){
	if(!defined('_PHP_FILE_')){
		if(IS_CGI){
			$_temp = explode('.php',$_SERVER['HTTP_SELF']);
			define('_PHP_FILE_',rtrim(str_replace($_SERVER['HTTP_HOST'],'',$temp[0].'.php'),'/'));
		}else{
			define('_PHP_FILE',rtrim($_SERVER['SCRIPT_NAME'],'/'));
		}
	}
	if(!defined('__ROOT__')){
		$_root = rtrim(_PHP_FILE,'/');
